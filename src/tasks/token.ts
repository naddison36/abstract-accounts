...
subtask("token-allowance", "Logs the amount of tokens a spender can transfer from an owner")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        log(
            `Spender ${spenderAddress} can transfer ${formatUnits(amount, tokenConfig.decimals)} ${tokenConfig.symbol} from ${ownerAddress}`
        )
    })
...
subtask("token-deploy", "Deploys a new mock ERC20 token")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const token = await deployContract(new ERC20__factory(signer), `Token ${name} (${symbol})`, constructorArguments)
